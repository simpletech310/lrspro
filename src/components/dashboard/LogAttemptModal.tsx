          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={captureGPS}
                className="bg-slate-100 text-slate-700 px-3 py-2 text-sm rounded-sm hover:bg-slate-200 flex items-center gap-2">
                📍 {gpsStatus === 'loading' ? 'Getting location...' : gpsStatus === 'captured' ? 'Recapture GPS' : 'Capture GPS'}
              </button>
              {gpsStatus === 'captured' && (
                <span className="text-emerald-600 text-xs">✓ {form.gps_lat?.toFixed(5)}, {form.gps_lng?.toFixed(5)}</span>
              )}
              {gpsStatus === 'error' && <span className="text-red-500 text-xs">Location unavailable</span>}
            </div>
          </div>
