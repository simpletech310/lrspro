...
import { CheckCircle2, FileText, MapPin, UploadCloud, AlertTriangle } from 'lucide-react'
...
export function CaseActions({ caseId, status, checklist, attempts, documents, userRole, staffId }: 
  ...
  // Handlers for interacting with API routes
        ...
        {activeTab === 'attempts' && (
          <div>
            <div className="mb-6 flex justify-between items-center">
              ...
              <button className="bg-[#C9A84C] text-[#0A1628] px-4 py-2 text-sm font-semibold rounded-sm hover:bg-[#E8C96A]">Log New Attempt</button>
            </div>
            {attempts?.length > 0 ? (
              <div className="space-y-4">
                {attempts.map(a => (
        ...
        {activeTab === 'documents' && (
          <div>
            <div className="mb-6 flex justify-between items-center">
              ...
              <button className="bg-[#0A1628] text-white px-4 py-2 text-sm font-semibold rounded-sm hover:bg-[#112240] flex items-center gap-2">
