...
export async function CTABanner() {
  const content = await getSectionContent('cta_banner')
  const heading = content.heading || 'Your Next Case Doesn\'t Have to Wait.'
  const description = content.description || 'Create your free account, place your order, and let us handle the legwork. Same-day rush service available for urgent filings and service of process.'

  return (
    <section className="bg-[#0A1628] py-16 sm:py-20 border-t border-white/10">
      <div className="max-w-4xl mx-auto px-6 text-center">
        <h2 className="font-display text-white text-3xl sm:text-4xl font-bold mb-4">{heading}</h2>
        <p className="text-slate-300 text-base sm:text-lg mb-8 sm:mb-10 max-w-2xl mx-auto">{description}</p>
