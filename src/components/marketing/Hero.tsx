      ...
      <div className="relative max-w-7xl mx-auto px-6 py-24 grid lg:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <div className="flex flex-wrap gap-6">
            {[{icon:Shield,label:'Licensed & Bonded'},{icon:Clock,label:'Same-Day Available'},{icon:MapPin,label:'Riverside County'}].map(({icon:Icon,label}, idx)=>(
              <motion.div 
                key={label}
        ...
        <div className="hidden lg:grid grid-cols-2 gap-4">
          {[{number:'500+',label:'Cases Completed'},{number:'24hr',label:'Rush Turnaround'},{number:'5',label:'Services Offered'},{number:'99%',label:'Delivery Rate'}].map(({number,label}, idx)=>(
            <motion.div 
              key={label}
