...
import { ArrowLeft, Download, Clock, FileText, CheckCircle2 } from 'lucide-react'
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-sm shadow-card p-6">
            ...
            <div className="relative pl-6 border-l-2 border-slate-100 space-y-8">
              {clientUpdates.map((u: any, i: number) => (
                <div key={u.id} className="relative">
                  <div className={`absolute -left-[33px] top-1 w-4 h-4 rounded-full border-2 bg-white ${i===0 ? 'border-[#C9A84C]' : 'border-slate-300'}`} />
                  <div className={`font-semibold ${i===0 ? 'text-[#0A1628]' : 'text-slate-600'}`}>{u.title}</div>
                  {u.content && <div className="text-sm text-slate-500 mt-1">{u.content}</div>}
                  <div className="text-xs text-slate-400 mt-1.5">{formatDateTime(u.created_at)}</div>
                </div>
              ))}
