    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {[{label:'Active Cases',value:count||0},{label:'Unassigned',value:unassignedCount||0},{label:'Total Cases (All Time)',value:totalCount||0}].map(s=>(
          <div key={s.label} className="bg-white rounded-sm border border-slate-200 p-4 sm:p-5 shadow-card">
            <div className="text-slate-500 text-xs sm:text-sm mb-1">{s.label}</div>
            <div className="font-display text-2xl sm:text-3xl font-bold text-[#0A1628]">{s.value}</div>
          </div>
        ))}
      </div>
        ...
        {cases && cases.length > 0 ? (
          <>
                ...
                <tbody className="divide-y divide-slate-100">
                  {cases.map(c => (
                    <tr key={c.id} className={`hover:bg-slate-50 transition-colors ${!c.assigned_staff_id ? 'bg-amber-50/50' : ''}`}>
                      ...
                      <td className="px-4 py-3"><span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[c.status as keyof typeof STATUS_COLORS]}`}>{STATUS_LABELS[c.status as keyof typeof STATUS_COLORS]}</span></td>
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {cases.map(c => (
                <Link key={c.id} href={`/dashboard/cases/${c.id}`} className={`block px-4 py-3.5 hover:bg-slate-50 transition-colors ${!c.assigned_staff_id ? 'bg-amber-50/50' : ''}`}>
                  ...
                  <div className="text-sm text-slate-600 mb-1">{c.service?.name}</div>
                  <div className="flex items-center justify-between text-xs text-slate-400">
                    <span>{c.client?.full_name}{c.client?.company ? ` · ${c.client.company}` : ''}</span>
